...
def save_config(module, result):
  ...
  if not module.check_mode:
    ...
    module.warn('Skipping command `save` due to check_mode.  Configuration not copied to non-volatile storage')
  ...
  if any((module.params['lines'], module.params['src'])):
    ...
    try:
      response = connection.get_diff(
        candidate=candidate, running=running, diff_match=match, diff_ignore_lines=diff_ignore_lines, path=path, diff_replace=replace
      )
  ...
  if module._diff:
    ...
    if contents is not None:
      ...
      if running_config.sha1 != base_config.sha1:
        result.update({'changed': True, 'diff': {'before': str(base_config), 'after': str(running_config)}})
